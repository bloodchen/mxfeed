...
import SnowflakeIdPkg from 'snowflake-id';
const SnowflakeId = SnowflakeIdPkg.default || SnowflakeIdPkg;

export class Util extends BaseService {
    async init(gl) {
        this.tokenPass = process.env.tokenPass || "2rnma5xsc3efx1Z$#%^09FYkRfuAsxTB"
        this.snowflake = new SnowflakeId({ mid: process.env.WORKER_ID || 1, offset: (2024 - 1970) * 31536000 * 1000 })
    }
    getClientIp(req) {
        let IP =
            //req.ip ||
