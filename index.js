...
async function main() {
    ...
    if (process.env.Modules.indexOf("posts") != -1) {
        const { Posts } = await import('./posts.js')
        await Posts.create(gl)
    }
    if (process.env.Modules.indexOf("follows") != -1) {
        const { Follows } = await import('./follows.js')
        await Follows.create(gl)
    }
    if (process.env.Modules.indexOf("queue") != -1) {
        const { QueueService } = await import('./queue.js')
        await QueueService.create(gl)
    }
...
async function regEndpoints() {
    ...
    await app.register(cors, { origin: true, credentials: true, methods: ['GET', 'POST', 'PUT', 'DELETE'], allowedHeaders: ['content-type', 'x-user-id'] });
    app.addHook("preHandler", async (req, res) => {
        console.log(req.url)
        // Trust user ID from upstream API Gateway
        const userIdHeader = req.headers['x-user-id'];
        if (userIdHeader) {
            req.uid = userIdHeader;
        }
        // Support test mode via query parameter
        if (req.query._testuid) {
            req.uid = req.query._testuid;
        }
    })
    app.get('/', (req, res) => {
        console.log(req.url)
